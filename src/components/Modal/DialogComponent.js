...
function DialogRender(id, data, classes, config_render, open, setOpen, modal_body, component_manager) {
  ...
  const handleClose = (opt) => {
    ...
    component_manager
      .getEventManager()
      .addEvent(id, 'closed', opt || {}, {});
    if (opt && opt.confirm) {
      component_manager
      .getEventManager()
      .addEvent(id, 'confirmed', opt || {}, {});
    }
  };
...
export function DialogComponent(props) {
  ...
  return DialogRender(props.id, props.data, classes, dialogConfig, open, setOpen, body, props.manager);
}
